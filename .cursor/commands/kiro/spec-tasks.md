...
- Reflect repository verification reality: TDD first, focused `pnpm test` usage, and broader `pnpm lint` / `pnpm test:gate` / `pnpm build` only when scope justifies them
...
- Prefer brownfield-safe sequencing that extends existing routes, services, hooks, auth guards, and validation modules before introducing new project-wide patterns
