...
## Overview
2-3 paragraphs max
**Purpose**: This feature delivers [specific value] to [target users] in the Gold Mustache product context.
**Users**: [Target user groups] will utilize this for [specific workflows].
**Impact** (if applicable): Changes the current [system state] by [specific modifications] while preserving existing app/router, auth, and service patterns.
...
- Existing tests, helpers, and route/action patterns to preserve before introducing new abstractions
...
> In this repository, typical stack choices include Next.js App Router, React 19, Prisma, Supabase, Tailwind, Zod, and Vitest. Only list the layers actually impacted by the feature.
...
Reference shared project helpers when relevant (for example API response mappers, auth guards, safe action wrappers) instead of inventing a parallel error convention.
...
- Include the RED -> GREEN -> REFACTOR expectation for new behavior.
- Identify the smallest relevant `pnpm test` target or focused test file strategy.
- When the feature changes broad runtime behavior, note whether `pnpm lint`, `pnpm build`, or `pnpm test:gate` should be part of final verification.
