...
- Reflect brownfield-safe sequencing when working inside existing modules or APIs.
...
- When a task changes behavior, include the relevant RED -> GREEN -> REFACTOR expectation in the task details or its sibling task structure.
- Reference the smallest meaningful verification step first (focused `pnpm test` target or specific suite) and escalate to broader verification only when risk justifies it.
- Use `pnpm lint`, `pnpm build`, or `pnpm test:gate` in task details only when the feature scope makes them meaningfully relevant.
...
- Prefer the repository's established stack and conventions unless the design explicitly introduces a justified deviation.
- Common boundaries in this project include App Router pages/routes, server actions, services, auth helpers, validation modules, and typed API helpers.
