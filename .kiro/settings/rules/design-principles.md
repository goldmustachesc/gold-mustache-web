...
- Prefer contracts that match the existing repository style (`@/` imports, typed helpers, focused interfaces)
...
- **Brownfield First**: Extend established project patterns before adding new architectural layers
...
- **Project Alignment**: Reuse route handlers, server actions, auth guards, API helpers, and Prisma/Supabase patterns already present in the codebase
...
- In this repository, default assumptions are Next.js App Router, React 19, Prisma, Supabase, Tailwind, Zod, Biome, and Vitest unless the feature explicitly needs something different.
...
- Testing notes should mention the expected verification path when the change affects behavior (focused `pnpm test`, broader gate, build, or manual route validation as appropriate).
