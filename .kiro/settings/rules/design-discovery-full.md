...
- Explicitly identify whether an existing App Router entry point, service module, auth helper, validation schema, or UI boundary should remain the owner of the new behavior
...
- Verification risk: what must be proven with focused tests, broader `pnpm` scripts, or manual flow checks before considering the feature done
